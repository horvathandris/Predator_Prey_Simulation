...
public class GraphView implements SimulatorView
{
    ...
    class GraphPanel extends JComponent
    {
        ...
        private HashMap<Class, Integer> lastVals;
        private int yMax;
        ...
        public void newRun()
        {
            ...
            for (Class claas : classes) {
                lastVals.put(claas, height);
            }
            repaint();
        }
        ...
        public void update(int step, Field field, FieldStats stats)
        {
            if (classes.size() >= 2) {
                ...
                for (Iterator<Class<?>> it = classes.iterator(); it.hasNext(); ) {
                    Class<?> claas = it.next();
                    int count = stats.getPopulationCount(field, claas);
                    ...
                    g.setColor(colors.get(claas));
                    g.drawLine(width-3, lastVals.get(claas), width-2, y);
                    lastVals.put(claas, y);
                }
        ...
        public void scaleDown()
        {
            ...
            for (Class claas : classes) {
                lastVals.put(claas, oldTop + (int) (lastVals.get(claas) * SCALE_FACTOR));
            }
